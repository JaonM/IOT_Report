...
def load_data(start_date, end_date, page_num):
    ...
    if page_num == -1:
        cursor.execute("select tb2.eui,tb2.temperature 温度,tb2.humidity 湿度, tb2.batt 电量,tb2.current 电流,tb2.voltage 电压,tb2.power 功率,tb2.ts 获取时间,tb1.name 传感器名称,tb3.name 所属设备,tb4.name 所属酒店\
                                            ...
                                            LEFT JOIN tb_customer tb4 on tb4.customer_id = tb1.customer_id where (tb2.eui='000D6F000ABFFB03' or tb2.eui='9896830000000008' or tb2.eui='9896830000000002' or tb2.eui = '9896830000000003' \
    ...
    for row in cursor.fetchall():
        result.append({'eui': row[0], 'temperature': row[1], 'humidity': row[2], 'battery': row[3],'current':row[4],'voltage':row[5] ,'time': row[7]})
    return result
